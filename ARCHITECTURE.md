...
| `se-css-temp-mobile.css` | Temp Board + モバイル（<=768px）の切替UI | ~6k |
| `presets.js` | 内蔵プリセット **`export const PRESETS`** のみ | ~65 |
| `game-se-tool-entry.js` | entry オーケストレータ（`se-*` を import、`window` 公開、初期化） | ~190 |
...
| `se-editor-ui.js` | エディタ UI（`updateParam` / `renderPresets` / `randomize` 等） | ~250 |
| `se-compare.js` | SE 比較モーダル（CMP スロット管理） | ~250 |
| `se-pseq.js` | ピッチシーケンサ（PSEQ） | ~250 |
| `se-arp.js` | アルペジエータ（ARP） | ~220 |
| `se-json-manager.js` | プリセット保存/読込（JSON / localStorage） | ~220 |
...
| `se-json-manager.js` | プリセット保存/読込/JSON入出力 |
...
- **ブレークポイント:** `max-width: 768px`（`se-css-temp-mobile.css` 末尾付近）
...
- **目的:** 左サイドバー/センター、センター/右パネルの間をドラッグで左右方向に調整。
- **DOM:** `game-se-tool.html` に `leftResizer` / `rightResizer` を追加。
- **CSS:** `se-css-core.css`
  - `.layout` の `grid-template-columns` を `--left-panel-w` / `--right-panel-w` + 固定リサイザー幅で構成。
  - `.panel-resizer` にドラッグ用の見た目（カーソル・境界線）を付与。
- **JS:** `game-se-tool-entry.js`
  - `pointerdown/move/up` で `--left-panel-w` / `--right-panel-w` を更新（モバイル時は無効）。
...
- `se-css-temp-mobile.css`: Temp Board とモバイル（<=768px）切替UI
...
- モバイル（≤768px）: 下部タブでプリセット / 編集 / ツールの3ペイン切替。
